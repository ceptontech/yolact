  ...
  CUDA_KERNEL_LOOP(index, count) {
    ...
    for (int ih = 0; ih < sample_per_part; ih++) {
      for (int iw = 0; iw < sample_per_part; iw++) {
        ...
        atomicAdd(offset_bottom_data_diff + bottom_index_base + y0 * width + x0,
                  q00 * diff_val);
        atomicAdd(offset_bottom_data_diff + bottom_index_base + y1 * width + x0,
                  q01 * diff_val);
        atomicAdd(offset_bottom_data_diff + bottom_index_base + y0 * width + x1,
                  q10 * diff_val);
        atomicAdd(offset_bottom_data_diff + bottom_index_base + y1 * width + x1,
                  q11 * diff_val);
...
std::tuple<at::Tensor, at::Tensor> dcn_v2_psroi_pooling_cuda_forward(
  ...
  AT_ASSERTM(channels == output_dim,
             "input channels and output channels must equal");
  auto pooled_height = pooled_size;
  auto pooled_width = pooled_size;

  auto out = at::empty({num_bbox, output_dim, pooled_height, pooled_width},
                       input.options());
  long out_size = num_bbox * output_dim * pooled_height * pooled_width;
  auto top_count = at::zeros(
      {num_bbox, output_dim, pooled_height, pooled_width}, input.options());
  ...
  AT_DISPATCH_FLOATING_TYPES(
      input.type(), "dcn_v2_psroi_pooling_cuda_forward", [&] {
        DeformablePSROIPoolForwardKernel<scalar_t><<<grid, block, 0, stream>>>(
            out_size, input.contiguous().data<scalar_t>(), spatial_scale,
            channels, height, width, pooled_height, pooled_width,
            bbox.contiguous().data<scalar_t>(),
            trans.contiguous().data<scalar_t>(), no_trans, trans_std,
            sample_per_part, output_dim, group_size, part_size, num_classes,
            channels_each_class, out.data<scalar_t>(),
...
std::tuple<at::Tensor, at::Tensor> dcn_v2_psroi_pooling_cuda_backward(
    ...
    const at::Tensor &trans, const at::Tensor &top_count, const int no_trans,
  ...
  auto input_grad =
      at::zeros({batch, channels, height, width}, out_grad.options());
  ...
  AT_DISPATCH_FLOATING_TYPES(
      out_grad.type(), "dcn_v2_psroi_pooling_cuda_backward", [&] {
        DeformablePSROIPoolBackwardAccKernel<scalar_t>
            <<<grid, block, 0, stream>>>(
                out_size, out_grad.contiguous().data<scalar_t>(),
                top_count.contiguous().data<scalar_t>(), num_bbox,
                ...
                bbox.contiguous().data<scalar_t>(),
                trans.contiguous().data<scalar_t>(), no_trans, trans_std,
                sample_per_part, group_size, part_size, num_classes,
                channels_each_class);
      });
  AT_CUDA_CHECK(cudaGetLastError());
  return std::make_tuple(input_grad, trans_grad);
