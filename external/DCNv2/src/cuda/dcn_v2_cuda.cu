...
at::Tensor dcn_v2_cuda_forward(const at::Tensor &input,
  ...
  // THCAssertSameGPU(THCudaTensor_checkGPU(state, 5, input, weight, bias,
  // offset, mask));
  ...
  auto ones = at::ones({batch, bias.sizes()[0], height_out, width_out},
                       input.options());
  auto columns = at::empty(
      {batch, channels * kernel_h * kernel_w, 1 * height_out * width_out},
      input.options());
  auto output =
      at::zeros({batch, channels_out, height_out, width_out}, input.options());
  ...
  modulated_deformable_im2col_cuda(
      ...
      /*data_im*/ input.const_data_ptr<scalar_t>(),
      /*data_offset*/ offset.const_data_ptr<scalar_t>(),
      /*data_mask*/ mask.const_data_ptr<scalar_t>(),
      /*batch_size*/ batch,
      /*channels*/ channels,
      /*height_im*/ height,
      /*width_im*/ width,
      /*height_col*/ height_out,
      /*width_col*/ width_out,
      /*kernel_h*/ kernel_h,
      /*kernel_w*/ kernel_w,
      /**/ pad_h,
  ...
  // torch implementation
  auto weight_flat =
      weight.view({channels_out, channels * kernel_h * kernel_w});
  auto product = at::matmul(weight_flat, columns);
  output = at::add(output,
                   product.view({batch, channels_out, height_out, width_out}));
...
std::vector<at::Tensor> dcn_v2_cuda_backward(
  ...
  auto columns =
      at::empty({channels * kernel_h * kernel_w, 1 * height_out * width_out},
                input.options());
  auto output =
      at::empty({batch, channels_out, height_out, width_out}, input.options());
  ...
  for (int b = 0; b < batch; b++) {
    ...
    auto grad_output_n_flat =
        grad_output_n.view({channels_out, height_out * width_out});
    columns = at::matmul(weight_flat, grad_output_n_flat);

    // gradient w.r.t. input coordinate data
    modulated_deformable_col2im_coord_cuda(
        c10::cuda::getCurrentCUDAStream(), columns.const_data_ptr<scalar_t>(),
        input_n.const_data_ptr<scalar_t>(), offset_n.const_data_ptr<scalar_t>(),
        mask_n.const_data_ptr<scalar_t>(), 1, channels, height, width,
        height_out, width_out, kernel_h, kernel_w, pad_h, pad_w, stride_h,
        stride_w, dilation_h, dilation_w, deformable_group,
        grad_offset_n.mutable_data_ptr<scalar_t>(),
        grad_mask_n.mutable_data_ptr<scalar_t>());

    // gradient w.r.t. input data
    modulated_deformable_col2im_cuda(
        c10::cuda::getCurrentCUDAStream(), columns.const_data_ptr<scalar_t>(),
        offset_n.const_data_ptr<scalar_t>(), mask_n.const_data_ptr<scalar_t>(),
        1, channels, height, width, height_out, width_out, kernel_h, kernel_w,
        pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w,
        deformable_group, grad_input_n.mutable_data_ptr<scalar_t>());

    // gradient w.r.t. weight, dWeight should accumulate across the batch and
    // group
    modulated_deformable_im2col_cuda(
        c10::cuda::getCurrentCUDAStream(), input_n.data_ptr<scalar_t>(),
        offset_n.data_ptr<scalar_t>(), mask_n.data_ptr<scalar_t>(), 1, channels,
        height, width, height_out, width_out, kernel_h, kernel_w, pad_h, pad_w,
    ...
    grad_weight =
        at::add(grad_weight,
                product.view({channels_out, channels, kernel_h, kernel_w}));
