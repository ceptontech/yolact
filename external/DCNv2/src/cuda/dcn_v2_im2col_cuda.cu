...
__device__ float dmcn_im2col_bilinear_cuda(const float *bottom_data,
...
__device__ float dmcn_get_coordinate_weight_cuda(
    ...
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        ...
        if (h_im > -1 && w_im > -1 && h_im < height && w_im < width) {
          ...
          // val = dmcn_im2col_bilinear_cuda(data_im_ptr, width, cur_height,
          // cur_width, map_h, map_w);
          val = dmcn_im2col_bilinear_cuda(data_im_ptr, width, height, width,
                                          h_im, w_im);
        }
    ...
    for (int dy = -2; dy <= 2; dy++) {
      for (int dx = -2; dx <= 2; dx++) {
        if (cur_h + dy >= 0 && cur_h + dy < height && cur_w + dx >= 0 &&
          ...
          float weight = dmcn_get_gradient_weight_cuda(
              cur_inv_h_data, cur_inv_w_data, cur_h + dy, cur_w + dx, height,
              width);
    ...
    for (int col_c = (offset_c / 2); col_c < channel_per_deformable_group;
      ...
      if (inv_h <= -1 || inv_w <= -1 || inv_h >= height || inv_w >= width) {
        ...
        mval += data_col_ptr[col_pos] *
                dmcn_im2col_bilinear_cuda(data_im_ptr + cnt * height * width,
